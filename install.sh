...
case "$OS" in
    ...
    Darwin*)
        if [[ "$ARCH" == "arm64" ]]; then
            ARCHIVE="hype-v1.11.1-darwin-arm64.tar.gz"
            echo -e "Detected: ${GREEN}macOS Apple Silicon (M1/M2)${NC}"
        elif [[ "$ARCH" == "x86_64" ]]; then
            ARCHIVE="hype-v1.11.1-darwin-amd64.tar.gz"
    ...
    CYGWIN*|MINGW*|MSYS*)
        if [[ "$ARCH" == "x86_64" ]]; then
            ARCHIVE="hype-v1.11.1-windows-amd64.zip"
